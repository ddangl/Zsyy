...
from fastapi import FastAPI, UploadFile, File, HTTPException, Query, Request, Response, Cookie, APIRouter
from fastapi.staticfiles import StaticFiles
from fastapi.responses import FileResponse, JSONResponse, HTMLResponse, RedirectResponse
from fastapi.middleware.cors import CORSMiddleware

# 将上级目录加入 path，以便 import processor 和 config
...
COOKIE_NAME = "daka_auth"
AUTH_TOKEN = "daka_auth_ok_2026"
PREFIX = "/daka"
# ==============================
...
app = FastAPI(title="打卡数据整理助手 API")
...
async def get_overview(file_id: str):
    ...
    display_names = {
        '姓名': '姓名',
        '工号': '工号',
        processor.COL_ORIGINAL_D1: '当日',
        config.TIME_OFF_WORK_COL: '下班时间',
        processor.COL_ORIGINAL_D2: '次日',
    }
...
async def save_changes(file_id: str, body: dict = None):
    ...
    try:
        ...
        for row_data in new_rows:
            ...
            eid = row_data.get("工号", "")
            off_work = row_data.get(config.TIME_OFF_WORK_COL, "")
            d1 = row_data.get(processor.COL_ORIGINAL_D1, "")
            d2 = row_data.get(processor.COL_ORIGINAL_D2, "")

            def log_fn2(text):
                logs.append(text)
            processor.patch_excel_data(file_path, name, eid, off_work, d1, d2, log=log_fn2)
